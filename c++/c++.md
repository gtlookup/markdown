...
```c++
void test(); // 如果方法体在调用者的后面，需要先声明，如果在调用者上面则不需要声明
...
- 方式2（常用）：将声明和实现写到一个文件中（.h文件），后将后缀名为 `.hpp`（只是约定名称，并不强制）
...
1. 比数组遍历慢，比数组点用空间大(多了`next`、`prev`指针)
